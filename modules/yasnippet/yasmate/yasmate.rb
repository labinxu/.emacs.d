...
if __FILE__ == $PROGRAM_NAME
  ...
  # Established the absolute pathnames of the files that will be used
  #
  original_dir = Dir.pwd
  yas_extra_substitutions_file = File.join(original_dir, opts.output_dir, ".yasmate-extra-substitutions.el")
  yas_unknown_substitutions_file = File.join(original_dir, opts.output_dir, ".yasmate-unknown-substitutions.el")
  yas_generated_menu_file = File.join(original_dir, opts.output_dir, ".yasmate-menu.el")
  yas_setup_el_file = File.join(original_dir, opts.output_dir, ".yas-setup.el")


  # puts "get this head #{head}"
  File.open yas_extra_substitutions_file, 'r' do |file|
              directive = nil
            ...
            end if File.readable? yas_extra_substitutions_file
  ...
  if opts.output_dir
    ...
    # Output unknown substitutions
    #
    File.open yas_unknown_substitutions_file, 'w' do |file|
    ...
    # Output the menu
    #
    File.open yas_generated_menu_file, 'w' do |file|
                file.puts ";; Translated menu"
                file.puts ";; "
                file.puts menustr
              end

    # Output a very basic .yas-setup.el file if it doesn't exist
    #
    File.open yas_setup_el_file, 'w' do |file|
                file.puts ";; skeleton .yas-setup.el file for #{modename}"
                file.puts ";; "
                file.puts "(load \"#{yas_generated_menu_file}\")"
                file.puts menustr
              end unless File.exist? yas_setup_el_file
